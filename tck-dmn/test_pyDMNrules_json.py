...
def convertJSON(thisValue):
    ...
    if len(yaccTokens) != 1:
        ...
        if thisValue == 'true':
            ...
            return thisValue
    elif yaccTokens[0].type == 'ATSTRING':
        thisString = thisValue[2:-1]
        isDateTime = re.fullmatch(SFeelLexer.DATETIME,thisString)
        if isDateTime:
            parts = thisString.split('@')
        ...
        if isDate:
            return dateutil.parser.parse(thisString).date()
        ...
        if dtd:
            ...
            if thisString[0] == '-':
                sign = -1
                thisString = thisString[1:]     # skip -
            thisString = thisString[1:]         # skip P
            days = seconds = milliseconds = 0
            if thisString[0] != 'T':          # days is optional
                parts = thisString.split('D')
                if len(parts[0]) > 0:
                    days = int(parts[0])
                thisString = parts[1]
            if len(thisString) > 0:
                thisString = thisString[1:]         # Skip T
                parts = thisString.split('H')
                if len(parts) == 2:
                    if len(parts[0]) > 0:
                        seconds = int(parts[0]) * 60 * 60
                    thisString = parts[1]
                parts = thisString.split('M')
                if len(parts) == 2:
                    if len(parts[0]) > 0:
                        seconds += int(parts[0]) * 60
                    thisString = parts[1]
                parts = thisString.split('S')
                if len(parts) == 2:
                    if len(parts[0]) > 0:
                        sPart = float(parts[0])
                        seconds += int(sPart)
...
def convertIn(thisValue, isTest):
    ...
    if len(yaccTokens) != 1:
        ...
        if yaccTokens[0].type == 'DTDURATION':
            return '@"' + thisValue + '"'
...
def collectListTests(listElement):
    ...
    logging.critical('listData:{}'.format(listData))
    return listData
...
Then process the file, named in the command line
    ...
    parser.add_argument ('-u', '--url', dest='url', default="http://localhost:5000", help='The URL of a decision service hosted by DecisionCentral (default=http://localhost:5000)')
    ...
    # Make Connect to the DecisionCentral
    urlBits = urlparse(url, allow_fragments=False)
    if urlBits.hostname is None:
        logging.critical('No hostname in URL - try full URL - e.g. http://host:port')
        logging.shutdown()
        sys.stdout.flush()
        sys.exit(EX_NOHOST)
    decisionCentralHost = urlBits.hostname
    logging.info('host:%s', decisionCentralHost)
    if urlBits.port is None:
        if urlBits.scheme == 'https':
            logging.info('https - using port 443')
            decisionCentralPort = 443
        else:
            decisionCentralPort = 80
    else:
        decisionCentralPort = urlBits.port
    logging.info('port:%s', decisionCentralPort)

    decisionCentralHeaders = {'Content-type':'application/json', 'Accept':'application/json'}
    logging.info('headers:%s', decisionCentralHeaders)
    try :
        if urlBits.scheme == 'https':
            logging.info('https - testing connection')
            decisionCentralConnection = client.HTTPSConnection(decisionCentralHost, decisionCentralPort)
        else:
            decisionCentralConnection = client.HTTPConnection(decisionCentralHost, decisionCentralPort)
        decisionCentralConnection.close()
        ...
        logging.critical('Cannot connect to the decisionCentral Service on host (%s) and port (%s). Error:%s', decisionCentralHost, decisionCentralPort, str(e))
        logging.shutdown()
        sys.stdout.flush()
    ...
    # for conformanceLevel in ['3']:
        ...
        # patterns = glob.glob(tckDir + '/tck-master/TestCases/compliance-level-' + conformanceLevel + '/1120*')
        ...
        for pattern in patterns:
            ...
            # Upload the DMN XML file
            try:
                request = requests.post(url + '/upload', files={'file': open(dmnFiles[0], 'rb')})
                request.raise_for_status()
            except:
                if not brief:
                    print('failed - Bad DMN file', dmnFiles[0], request.text)
                logging.critical('failed - Bad DMN file %s - %s', dmnFiles[0], request.text)
                badDMN = True
            ...
            if len(tests) > 0:
                for i in range(len(tests)):
                    ...
                    if thisPattern in fullTests:         # Some tests require previous decision outputs as inputs to subsequent decisions
                        logging.critical('Posting data:{}'.format(data))
                        request = requests.post(url + '/api/' + thisPattern, headers=decisionCentralHeaders, json=data)
                    else:
                        request = requests.post(url + '/api/' + thisPattern + '_table/' + decisionNames[i], headers=decisionCentralHeaders, json=data)
                    if request.status_code != requests.codes.ok:
                        if not brief:
                            print('failed - DecisionCentral error', dmnFiles[0], '-', request.text)
                        logging.critical('failed - Bad DMN file %s - %s', dmnFiles[0], request.text)
                        continue
                    try:
                        ...
                        try:
                            newData = json.loads(request.text)
                        except:
                            if not brief:
                                print('failed - DecisionCentral error', dmnFiles[0], '-', request.text)
                            else:
                                print('{}\t{}\t{}\tfailed\t{}'.format(conformanceLevel, thisPattern, testNames[i], 'bad JSON'))
                                logFailure(tests, results, data, {}, {})
                                failed = True
                            logging.critical('failed - Bad JSON %s - %s', dmnFiles[0], request.text)
                            continue
                    ...
                    if 'errors' in status:
                        ...
                        try:
                            ...
                            logging.debug('\tnewData\t%s', str(newData).encode(errors='replace'))
                    foundResults = []
                    if (len(executedRule) == 3) and (not isinstance(executedRule[0], list)):
                            thisTable = executedRule[1]
                            if thisPattern in fullTests:         # Some tests require previous decision outputs as inputs to subsequent decisions
                        ...
                        for j in range(len(executedRule)):
                            thisTable = executedRule[j][1]
                            if thisPattern in fullTests:         # Some tests require previous decision outputs as inputs to subsequent decisions
                                if thisTable in decisionNames:
                                    foundResults.append(thisTable)
                            else:
                    ...
                    if len(missing) > 0:
                        ...
                        failed = True
                        logging.info('Test: %s missing results for Decision Table %s - %s', thisPattern, testNames[i], missing)
                        logFailure(tests, results, data, result, status)
                        continue
                    for variable in results[i]:         # Check each variable
                        ...
                        if isinstance(value, list):     # We are expecting a list
                            if not isinstance(result[variable], list):
                                if not brief:
                                    print('Test:', testNames[i], 'failed - returned value', result[variable], 'for', variable, 'when', value, 'expected')
                                else:
                                    print('{}\t{}\t{}\tfailed\treturned value {} for variable {} when value {} expected'.format(conformanceLevel, thisPattern, testNames[i], result[variable], variable, value))
                                    failed = True
                                logging.info('Test: %s failed - returned value %s for %s when %s expected', testNames[i], result[variable],  variable, value)
                                logFailure(tests, results, data, newData, status)
                                break
                            for j in range(len(value)):     # Check each of the list items
                                thisValue = value[j]
                                if j >= len(result[variable]):
                                    if not brief:
                                        print('Test:', testNames[i], 'failed - returned value', result[variable], 'for', variable, 'when', value, 'expected')
                                    else:
                                        print('{}\t{}\t{}\tfailed\treturned value {} for variable {} when value {} expected'.format(conformanceLevel, thisPattern, testNames[i], result[variable], variable, value))
                                    ...
                                    logFailure(tests, results, data, newData, status)
                                    break
                                thisJSON = convertJSON(result[variable][j])
                                if isinstance(thisValue, float):
                                    thisValue = int(thisValue * (10 ** decimalDigits) + 0.5)/(10.0 ** decimalDigits)
                                    if isinstance(thisJSON, float) or isinstance(thisJSON, int):
                                        thisJSON = int(float(thisJSON) * (10 ** decimalDigits) + 0.5)/(10.0 ** decimalDigits)
                                if thisJSON != thisValue:
                                    if not brief:
                                        print('Test:', testNames[i], 'failed - returned value', thisJSON, 'for', variable, 'when', thisValue, 'expected')
                                    else:
                                        print('{}\t{}\t{}\tfailed\treturned value {} for variable {} when value {} expected'.format(conformanceLevel, thisPattern, testNames[i], thisJSON, thisValue, value))
                                        failed = True
                                    logging.info('Test: %s failed - returned value %s for %s When %s expected', testNames[i], thisJSON,  thisValue, value)
                                    logFailure(tests, results, data, newData, status)
                                    break
                            else:
                                continue
                            ...
                            if not isinstance(result[variable], dict):
                                if not brief:
                                    print('Test:', testNames[i], 'failed - returned value', result[variable], 'for', variable, 'when', value, 'expected')
                                else:
                                    print('{}\t{}\t{}\tfailed\treturned value {} for variable {} when value {} expected'.format(conformanceLevel, thisPattern, testNames[i], result[variable], variable, value))
                            ...
                            for thisVariable in value:      # Check each of the dictionary items
                                ...
                                if thisVariable not in result[variable]:
                                    if not brief:
                                        try:
                                ...
                                if isinstance(thisValue, list):       # We are expecting a dictionary of lists
                                    if not isinstance(result[thisVariable], list):
                                        if not brief:
                                            try:
                                                print('Test:', testNames[i], 'failed - returned value', result[thisVariable], 'for', thisVariable, 'when', thisValue, 'expected')
                                            except:
                                                print('Test:', testNames[i], 'failed - returned value', str(result[thisVariable]).encode(errors='replace'), 'for', str(thisVariable).encode(errors='replace'), 'when', str(thisValue).encode(errors='replace'), 'expected')
                                        else:
                                            try:
                                                print('{}\t{}\t{}\tfailed\treturned value {} for variable {} when value {} expected'.format(conformanceLevel, thisPattern, testNames[i], result[thisVariable], thisVariable, thisValue))
                                            except:
                                                print('{}\t{}\t{}\tfailed\treturned value {} for variable {} when value {} expected'.format(conformanceLevel, thisPattern, testNames[i], str(result[thisVariable]).encode(errors='replace'), str(thisVariable).encode(errors='replace'), str(thisValue).encode(errors='replace')))
                                            failed = True
                                        try:
                                            logging.info('Test: %s failed - returned value %s for %s when %s expected', testNames[i], result[thisVariable],  thisVariable, thisValue)
                                        except:
                                            logging.info('Test: %s failed - returned value %s for %s when %s expected', testNames[i], str(result[thisVariable]).encode(errors='replace'),  str(thisVariable).encode(errors='replace'), str(thisValue).encode(errors='replace'))
                                        logFailure(tests, results, data, newData, status)
                                        break
                                    for j in range(len(thisValue)):
                                        thisOneValue = thisValue[j]
                                        if j >= len(result[thisVariable]):
                                            if not brief:
                                                print('Test:', testNames[i], 'failed - returned value', result[variable], 'for', variable, 'when', value, 'expected')
                                            else:
                                                print('{}\t{}\t{}\tfailed\treturned value {} for variable {} when value {} expected'.format(conformanceLevel, thisPattern, testNames[i], result[variable], variable, value))
                                    ...
                                    thisJSON = convertJSON(result[variable][thisVariable])
                                    if isinstance(thisValue, float):
                                        thisValue = int(thisValue * (10 ** decimalDigits) + 0.5)/(10.0 ** decimalDigits)
                                        if isinstance(thisJSON, float) or isinstance(thisJSON, int):
                                            thisJSON = int(float(thisJSON) * (10 ** decimalDigits) + 0.5)/(10.0 ** decimalDigits)
                                    if thisJSON != thisValue:
                                        if not brief:
                                            try:
                                                print('Test:', testNames[i], 'failed - returned value', thisJSON, 'for', thisVariable, 'when', thisValue, 'expected')
                                            except:
                                                print('Test:', testNames[i], 'failed - returned value', str(thisJSON).encode(errors='replace'), 'for', str(thisVariable).encode(errors='replace'), 'when', str(thisValue).encode(errors='replace'), 'expected')
                                        else:
                                            try:
                                                print('{}\t{}\t{}\tfailed\treturned value {} for variable {} when value {} expected'.format(conformanceLevel, thisPattern, testNames[i], thisJSON, thisVariable, thisValue))
                                            except:
                                                print('{}\t{}\t{}\tfailed\treturned value {} for variable {} when value {} expected'.format(conformanceLevel, thisPattern, testNames[i], str(thisJSON).encode(errors='replace'), str(thisVariable).encode(errors='replace'), str(thisValue).encode(errors='replace')))
                                            failed = True
                                        try:
                                            logging.info('Test: %s failed - returned value %s for %s when %s expected', testNames[i], thisJSON,  thisVariable, thisValue)
                                        except:
                                            logging.info('Test: %s failed - returned value %s for %s when %s expected', testNames[i], str(thisJSON).encode(errors='replace'),  str(thisVariable).encode(errors='replace'), str(thisValue).encode(errors='replace'))
                                        logFailure(tests, results, data, newData, status)
                                        break
                            else:
                                continue
                            ...
                            if variable not in result:
                                ...
                                if not brief:
                                    break
                            thisJSON = convertJSON(result[variable])
                            if isinstance(value, float):
                                value = int(value * (10 ** decimalDigits) + 0.5)/(10.0 ** decimalDigits)
                                if isinstance(thisJSON, float) or isinstance(thisJSON, int):
                                    thisJSON = int(float(thisJSON) * (10 ** decimalDigits) + 0.5)/(10.0 ** decimalDigits)
                            elif isinstance(value, datetime.time) and isinstance(thisJSON, datetime.time):       # check for equivalent timezones (thisResult == +23)
                                resTime = thisJSON
                            ...
                            if thisJSON != value:
                                if not brief:
                                    try:
                                        print('Test:', testNames[i], 'failed - returned value', thisJSON, 'for', variable, 'when', value, 'expected')
                                    except:
                                        print('Test:', testNames[i], 'failed - returned value', thisJSON, 'for', str(variable).encode(errors='replace'), 'when', str(value).encode(errors='replace'), 'expected')
                                else:
                                    try:
                                        print('{}\t{}\t{}\tfailed\treturned value {} for variable {} when value {} expected'.format(conformanceLevel, thisPattern, testNames[i], thisJSON, variable, value))
                                    except:
                                        print('{}\t{}\t{}\tfailed\treturned value {} for variable {} when value {} expected'.format(conformanceLevel, thisPattern, testNames[i], str(thisJSON).encode(errors='replace'), str(variable).encode(errors='replace'), value))
                                    failed = True
                                try:
                                    logging.info('Test: %s failed - returned value %s for %s when %s expected', testNames[i], thisJSON,  variable, value)
                                except:
                                    logging.info('Test: %s failed - returned value %s for %s when %s expected', testNames[i], str(thisJSON).encode(errors='replace'),  str(variable).encode(errors='replace'), str(value).encode(errors='replace'))
                                logFailure(tests, results, data, newData, status)
                                if not brief:
                                    break
                    else:
            ...
            request = requests.get(url + '/delete/' + thisPattern)
            if request.status_code != requests.codes.ok:
                if not brief:
                    print('failed - DecisionCentral error', dmnFiles[0], '-', request.text)
                logging.critical('failed to delete DMN service %s - %s', dmnFiles[0], request.text)
